// Pure email-template rendering — no I/O, no `server-only` — so it can be
// unit/snapshot tested without booting nodemailer or Next's server context.

/** Optional agent branding — when present, the email is white-labelled. */
export interface MailBrand {
  companyName?: string | null;
  logo?: string | null;
  /** Reply-To (agent's support inbox); the SMTP From stays the platform sender. */
  replyTo?: string | null;
}
...
export function flightConfirmationSubject(d: FlightConfirmationData): string {
  return `Booking Confirmed — ${d.origin} → ${d.destination} (PNR: ${d.pnr})`;
}
...
export function flightConfirmationHtml(d: FlightConfirmationData): string {
  const paxList = d.passengerNames
    .map((n) => `<li style="margin:4px 0;color:#374151;">${escapeHtml(n)}</li>`)
    .join("");
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="padding:18px 40px;text-align:center;border-top:1px solid #e5e7eb;">
            <div style="font-size:11px;color:#9ca3af;">This is an automated confirmation from ${brandName}.${d.brand?.replyTo ? " Reply to this email for support." : " Please do not reply to this email."}</div>
          </td>
        </tr>
